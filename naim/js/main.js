...
if (accor_dt) {
  let accor_desc = document.querySelectorAll('dd');
  accor_dt.forEach(el => el.addEventListener('click', onAccordionClicked, false));
  initAccordion();
  function initAccordion() {
    accor_desc.forEach(el => {
      if (el.classList.contains('hidden')) {
  ...
  function onAccordionClicked(e) {
    if (e.currentTarget.closest('.faq') || e.currentTarget.closest('.doc-page')) {
      accor_desc.forEach(el => {
        if (!el.classList.contains('hidden')) {
...
let cards_train = document.querySelectorAll('.card-train');
if (cards_train) {
  let filter_items = document.querySelectorAll('.filter-item');
  filter_items.forEach(el => {
    el.addEventListener('click', showChosenCards, false);
  });
  function showChosenCards(e) {
    hideAllCards();
    let output_cat = e.currentTarget.querySelector('span').innerText;
    switch (output_cat) {
      case 'Семинары':
        {
          cards_train.forEach(el => {
            if (el.querySelector('.label.seminar')) {
      ...
      case 'Вебинары':
        {
          cards_train.forEach(el => {
            if (el.querySelector('.label.webinar')) {
              el.classList.remove('hidden');
            }
          });
        }
      ...
      case 'Курсы':
        {
          cards_train.forEach(el => {
            if (el.querySelector('.label.online') || el.querySelector('.label.recorded')) {
              el.classList.remove('hidden');
            }
          });
        }
...
if (check_slider_main) {
  ...
  var slider_main = new _splidejs_splide__WEBPACK_IMPORTED_MODULE_0__["default"]('.slider-main', {
    type: 'loop',
    // autoplay: true,
    interval: 3000,
    speed: 1000,
...
if (check_slider_news) {
  ...
  var slider_news = new _splidejs_splide__WEBPACK_IMPORTED_MODULE_0__["default"]('.slider-news', {
    type: 'loop',
    autoplay: true,
    interval: 3000,
    speed: 1000,
...
let book_slider = document.getElementById('book-slider');
if (book_slider) {
  let splide = new _splidejs_splide__WEBPACK_IMPORTED_MODULE_0__["default"]('#book-slider', {
    pagination: false
  });
  let thumbnails = document.getElementsByClassName('book-slider__thumbnail');
  let current;
...
let book_tabs = document.querySelector('.book-tabs');
if (book_tabs) {
  let btns = book_tabs.querySelectorAll('.book-tabs__nav-trigger');
  btns.forEach(btn => btn.addEventListener('click', handleNavTrigger));
  btns.forEach((btn, index) => {
    btn.setAttribute('id', `tab-label-${index + 1}`);
    btn.setAttribute('aria-controls', `tab-panel-${index + 1}`);
    if (index === 0) {
  ...
  let panels = book_tabs.querySelectorAll('.book-tabs__panel');
  panels.forEach((panel, index) => {
    panel.setAttribute('aria-labelledby', `tab-label-${index + 1}`);
    panel.setAttribute('id', `tab-panel-${index + 1}`);
  ...
  function handleNavTrigger(e) {
    ...
    let current = book_tabs.querySelector(`[aria-labelledby=${id}]`);
