...
if (gal) {
  new simplelightbox__WEBPACK_IMPORTED_MODULE_0__["default"]('.simple-gallery a', {
    overlayOpacity: .85
    //   spinner: false,
    // nav: false,
    //   close: false,
...
//----- ABOUT SLIDER
let about_slider = document.querySelector("[data-about-slider]");
if (about_slider) {
  let ww = window.innerWidth;
  let slides, prev, next, slides_num, slides_to_show;
  let slide_links = about_slider.querySelectorAll('.about-slider__img');
  slide_links.forEach(link => link.addEventListener('click', handleLink));
  function handleLink(e) {
    let target = e.currentTarget;
    if (target.parentElement.classList.contains('wide')) {
      let text = target.lastElementChild.innerText;
      sessionStorage.setItem('cat-name', text);
    } else {
      e.preventDefault();
      if (prev.disabled === true) prev.disabled = false;
      if (next.disabled === true) next.disabled = false;
      let wide_slide = about_slider.querySelector('.wide');
  ...
  function onNext(e) {
    if (prev.disabled === true) prev.disabled = false;
    let slides_vis = Array.from(about_slider.querySelectorAll(".wide,.narrow"));
    let slide_vis_last = slides_vis[slides_vis.length - 1];
    if (slide_vis_last === slides[slides_num - 1] && slide_vis_last.classList.contains('wide')) return;
    if (slide_vis_last.classList.contains("wide")) {
      rewriteClassesNextDirection(slides_vis);
      let last = about_slider.querySelector('.wide');
      if (last === slides[slides_num - 1]) {
        e.currentTarget.disabled = true;
      }
      return;
    }
    let slide_wide = about_slider.querySelector(".wide");
    slide_wide.classList.remove("wide");
    slide_wide.classList.add("narrow");
    let slide_next = slide_wide.nextElementSibling;
    slide_next.classList.remove("narrow");
  ...
  function onPrev(e) {
    ...
    if (slide_vis_first.classList.contains("wide")) {
      rewriteClassesPrevDirection(slides_vis);
      let first = about_slider.querySelector('.wide');
      if (first === slides[0]) {
        e.currentTarget.disabled = true;
      }
      return;
...
if (filter_tabs) {
  ...
  function createArrayWithActiveOptions() {
    filter_tags = [];
    var options = document.querySelectorAll('.filter-option.active');
    options.forEach(option => {
      filter_tags.push(option.innerText);
    });
    console.log(options, filter_tags);
  }
